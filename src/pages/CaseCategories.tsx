...
const CaseCategories = () => {
  ...
  const [editItem, setEditItem] = useState<CaseCategory | null>(null);
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const { toast } = useToast();

  const load = async () => { const res = await caseCategoriesAPI.getAll(); setItems(res.data); };
  useEffect(() => { load(); }, []);

  const resetForm = () => { setName(""); setDescription(""); setEditItem(null); };

  const handleSave = async () => {
    try {
      const data = { name, description: description || undefined };
      if (editItem) { await caseCategoriesAPI.update(editItem.id, data); toast({ title: "Category updated" }); }
      else { await caseCategoriesAPI.create(data); toast({ title: "Category created" }); }
  ...
  const handleDelete = async (id: number) => {
    ...
    try { await caseCategoriesAPI.delete(id); toast({ title: "Deleted" }); load(); }
    catch (err: any) { toast({ title: "Error", description: err.response?.data?.detail || "Cannot delete", variant: "destructive" }); }
  };

  const openEdit = (item: CaseCategory) => { setEditItem(item); setName(item.name); setDescription(item.description || ""); setOpen(true); };

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-foreground md:text-3xl">Case Categories</h1><p className="mt-1 text-muted-foreground">Manage case categories</p></div>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) resetForm(); }}>
          <DialogTrigger asChild><Button className="gradient-cta text-primary-foreground"><Plus className="mr-2 h-4 w-4" />Add Category</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>{editItem ? "Edit Category" : "Add Category"}</DialogTitle></DialogHeader>
            <div className="space-y-4 pt-4">
              <div><label className="mb-1.5 block text-sm font-medium">Name</label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Category name" /></div>
          ...
          <tbody className="divide-y divide-border">
            {items.map((item) => (
              <tr key={item.id} className="transition-colors hover:bg-secondary/30">
                ...
                <td className="px-4 py-3 text-sm font-medium text-foreground"><div className="flex items-center gap-2"><FolderTree className="h-4 w-4 text-muted-foreground" />{item.name}</div></td>
            ...
            {items.length === 0 && (<tr><td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">No categories found</td></tr>)}
          </tbody>
        </table>
      </div>
    </motion.div>
